...
const envSchema = z.object({
  JELLYFIN_PORT: z.string().transform(Number).pipe(z.number().int().positive()).default('8096'),
  JELLYFIN_HTTPS_PORT: z.string().transform(Number).pipe(z.number().int().positive()).default('8920'),
  HTTP_PORT: z.string().transform(Number).pipe(z.number().int().positive()).default('80'),
  HTTPS_PORT: z.string().transform(Number).pipe(z.number().int().positive()).default('443'),
